...
input_str = """Game 1: 1 blue; 4 green, 5 blue; 11 red, 3 blue, 11 green; 1 red, 10 green, 4 blue; 17 red, 12 green, 7 blue; 3 blue, 19 green, 15 red
...
parsed_lines = [re.split(';|,', line.replace(",","_").replace(":",",")) for line in lines]
counter = sum(int(re.search(r"\d+", line[0]).group()) for line in parsed_lines)                     ## Starting by assuming all tirage are "valable"

for i in range(0, len(parsed_lines)):
    print(parsed_lines[i])
    for elem1 in parsed_lines[i]:
        if any('blue' in elem and int(re.search(r"\d+", elem).group()) > 14 or
                'green' in elem and int(re.search(r"\d+", elem).group()) > 13 or
                'red' in elem and int(re.search(r"\d+", elem).group()) > 12 for elem in re.split('_', elem1)):
            counter -= int(re.search(r"\d+", parsed_lines[i][0]).group())
            break
print(counter)
'''
## Part 2 -- Counting the sum of red*green*blue necessary for each tirage

parsed_lines = [re.split(';|,', line.replace(",","_").replace(":",",")) for line in lines]
sum_power_counter = 0

for i in range(0, len(parsed_lines)):
    current_bloc = {"red": 0, "blue": 0, "green": 0}
    for elem1 in parsed_lines[i]:
        for elem in re.split('_', elem1):
            if ("red" in elem and int(re.search(r"\d+", elem).group()) > current_bloc["red"]):
                current_bloc["red"] = int(re.search(r"\d+", elem).group())
            elif ("blue" in elem and int(re.search(r"\d+", elem).group()) > current_bloc["blue"]):
                current_bloc["blue"] = int(re.search(r"\d+", elem).group())
            elif ("green" in elem and int(re.search(r"\d+", elem).group()) > current_bloc["green"]):
                current_bloc["green"] = int(re.search(r"\d+", elem).group())
    sum_power_counter += current_bloc["red"] * current_bloc["blue"] * current_bloc["green"]

print(sum_power_counter)
'''