"""
    ...
    rejected (R) [Meaning : NOT Considered]
    accepted (A) [Meaning : Considered]
"""

"Piece in format : {x:value, m:value, a:value, s:value}"
"Algorithm_Dict : Algorithm_name: string without the {}"
"Algortihm in format : "

Input="""cq{a<289:R,a<499:A,m>826:A,A}
...
Algorithm_Dict = {line[:line.find('{')]: line[line.find('{')+1: -1] for line in Input.split('\n\n')[0].split('\n')}

Pieces = [line[1:-1] for line in Input.split('\n\n')[1].split('\n')]
Pieces = [{composant[0]:composant[2:] for composant in Piece.split(',')} for Piece in Pieces]
Piece_accepted = []

print(f' This is Pieces after the formating : {Pieces}')
print(f' This is Instructions before the formating : {Algorithm_Dict}')

def Go_Through_Algorithm(piece, algorithm_string):
    for instruction in algorithm_string.split(','):
        part = instruction.split(':')
        if len(part) == 1:
            if part[0] == 'A':
                Piece_accepted.append(piece)
                return
            elif part[0] == 'R':
                return
            else:
                Go_Through_Algorithm(piece, Algorithm_Dict[part[0]])
        else:
            condition, result = part[0], part[1]
            if (eval(piece[condition[0]] + condition[1:])):
                if result == 'A':
                    Piece_accepted.append(piece)
                    return
                elif result == 'R':
                    ...
                    return Go_Through_Algorithm(piece, Algorithm_Dict[result])

for Piece in Pieces:
    Go_Through_Algorithm(Piece, Algorithm_Dict['in'])

print(f' This are the Piece_accepted : {Piece_accepted} \n')

Result_Part1 = sum([int(value) for piece in Piece_accepted for value in piece.values()])
print("This is Result_Part1 : ", Result_Part1, '\n')