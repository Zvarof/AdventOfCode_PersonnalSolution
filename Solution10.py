...
|JL-JJ-|.FJ-LL7LF7-F-JL--LJL|---|JLJJJ.|-JLLJ|J.FJ.FLLJJLJ-J|L-.LF-J-LJLLJ-7.L--L..LLJ-.LJJFJ---JL7J.LFJLF-J7.|-JJLJ.|J-JJ|L7J|.|.LL-7.|J.-J""".split('\n')

l = len(Input[0])
h = len(Input)
Start = [(index, line.find("S")) for index, line in enumerate(Input) if line.find("S") != -1][0]
...
dict_interaction = {
    ...
    '7': [(1, 0), (0, -1)],
    'F': [(1, 0), (0, 1)],
    'S': [(-1, 0), (0, -1)]                 # Need to be setup with the (only?) 2 options, not all 4.
    }

step = 0
prev_coordonate = []

def find_next(symbol, coordonate, step, prev_coordonate, Start):
    if step == 2:
        prev_coordonate.remove(Start)

    for possibilitie in dict_interaction[symbol]: 
        Row = coordonate[0] + possibilitie[0]             # The next one
        Column = coordonate[1] + possibilitie[1]          # The next one
        
        if (Row >= h or Row<0 or Column >= l or Column<0):
            continue

        next_Symbol_of_position = Input[Row][Column]
        next_coordonate = (Row, Column)

        if next_Symbol_of_position == '.':
            continue

        elif (next_Symbol_of_position == 'S' and next_coordonate not in prev_coordonate):
            prev_coordonate.append(coordonate)
            print("This is the TOTAL number of step :", step + 1, '\n')
            # return step + 1
            return prev_coordonate
        
        elif (next_Symbol_of_position != 'S' and next_coordonate not in prev_coordonate):
            return find_next(next_Symbol_of_position, (Row, Column), step + 1, prev_coordonate + [coordonate], Start)
            
full_coordinate = find_next('S', Start, step, prev_coordonate, Start)
full_coordinate.insert(0, Start)
full_coordinate.append(Start)
print("This is full_coordinate : ", full_coordinate, '\n', '\n', '\n', '\n', '\n', '\n')


'''
def fill_algorithm(Start, figure_shape, already_tested, inside_coordonate):
    already_tested.add(Start)
    inside_coordonate.add(Start)
    y = Start[0]
    x = Start[1]
    for new_pos in [(y+1, x), (y-1, x), (y, x+1), (y, x-1)]:
        if (new_pos not in figure_shape and new_pos not in already_tested): # and new_pos[0]<140 and new_pos[1]<140 and new_pos
            already_tested.add(new_pos)
            return fill_algorithm(new_pos, figure_shape, already_tested, inside_coordonate)
    return inside_coordonate

Test = (60, 60)
already_tested = set()
inside_coordonate = set()
'''

Point_to_test = [(x, y) for x in range(140) for y in range(140) if (x, y) not in full_coordinate]
MyPath = mpp.Path(full_coordinate)
# print("My path : ", MyPath)
ShouldIKeep = MyPath.contains_points(Point_to_test)

Number_of_kept = sum(ShouldIKeep)
print(Number_of_kept)                       # Solution